...
void GameThread(void*)
{
	...
	while(true)
	{
		...
		/*if (GetAsyncKeyState(VK_NUMPAD1))
		{
			...
			{
				if ((int)g_pGameLevel->ObjectList.objects[i] > 0x400000)
				{
					CObject * obj = g_pGameLevel->ObjectList.objects[i];
					//LogHandle->Write("obj name: %s", obj->cNameSect_str());
					if (strstr(obj->cNameSect_str(), "sim_default_") != NULL)
					{
						LogHandle->Write("Stalker id: %u", obj->ID());
						_asm {
							mov     ecx, [obj + 5ECh]
							mov     eax, [ecx]
							mov     edx, [eax + 8]
