...
void CHook::Nop(unsigned long addr, unsigned int size)
{
	DWORD oldp;
	if (VirtualProtect((LPVOID)addr, size, PAGE_EXECUTE_READWRITE, &oldp))
	{
		for (unsigned int i = 0; i < size; i++) *(unsigned char*)(addr + i) = 0x90;
	}
}


void CHook::FloatToHex(unsigned long addr, float val)
{
	unsigned int hex;
	RtlCopyMemory(&hex, &val, sizeof(float));
	unsigned char bytes[4];
	bytes[0] = (unsigned char)(hex);
	bytes[1] = (unsigned char)(hex >> 8);
	bytes[2] = (unsigned char)(hex >> 16);
	bytes[3] = (unsigned char)(hex >> 24);
