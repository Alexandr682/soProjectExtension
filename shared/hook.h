...
public:
	...
	void Nop(unsigned long addr, unsigned int size);
	void FloatToHex(unsigned long addr, float val);
	...
	virtual void Add_money_mp(ClientID cl_id) {}
	virtual IConsole_Command* CreateConsoleCommand(char* name,void* func) {return NULL;}
	virtual CCC_Integer* CreateConsoleCommandInt(char const* N, int* V, int _min, int _max) { return NULL; }
	virtual CCC_Float* CreateConsoleCommandFloat(char const* N, float* V, float _min, float _max) { return NULL; }
